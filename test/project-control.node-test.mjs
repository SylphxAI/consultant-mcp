...
test('package scripts expose reproducible local gates and protected publication metadata', () => {
  ...
  assert.equal(
    pkg.scripts.verify,
    'npm run typecheck && npm test && npm run build && npm run test:rust && npm run test:parity && npm run test:http-transport && npm run check:no-ts-http-backend && npm run check:no-ts-stdio-backend && npm run check:no-ts-tools-backend && npm run pack:beta'
  )
