...
if command -v sha256sum >/dev/null 2>&1; then
  BEHAVIOR_SPEC_HASH="$(sha256sum "$REPO_ROOT/scripts/differential/fixtures/consultant-mcp-corpus.json" | awk '{print $1}')"
else
  BEHAVIOR_SPEC_HASH="$(shasum -a 256 "$REPO_ROOT/scripts/differential/fixtures/consultant-mcp-corpus.json" | awk '{print $1}')"
fi
...
jq -n \
  ...
  '{
    ...
    promotionHold: true,
    promotionHoldRef: "rej-010",
    authorityRustPromotions: 0,
    tsDeletedPromotions: 0
  }' >"$ARTIFACT"
