...
if [[ -f "$ROOT/src/server.ts" ]]; then
	echo "[check-ts-adapter-deleted] src/server.ts must be deleted when transport/stdio-ts-adapter is ts_deleted" >&2
	exit 1
fi

if [[ -f "$ROOT/dist/server.js" ]]; then
	echo "[check-ts-adapter-deleted] dist/server.js must be deleted when transport/stdio-ts-adapter is ts_deleted" >&2
	exit 1
fi
...
if grep -q 'CONSULTANT_MCP_TRANSPORT:-}" == "ts"' "$BIN"; then
	echo "[check-ts-adapter-deleted] bin must not route CONSULTANT_MCP_TRANSPORT=ts after ts_deleted" >&2
	exit 1
fi
