 ...
 *   artifacts/consultant-native-<rust-target>/consultant-mcp-server
 *
 * Writes into:
 *   npm/<platform-key>/consultant-mcp-server
 *
 * Fail-closed: requires all configured platforms unless CONSULTANT_MULTIARCH_PARTIAL=1.
 */
...
const PLATFORMS = [
  {
    ...
    artifactNames: ["consultant-native-aarch64-apple-darwin"],
  },
  {
    key: "darwin-x64",
    target: "x86_64-apple-darwin",
    artifactNames: ["consultant-native-x86_64-apple-darwin"],
  },
  {
    key: "linux-x64-gnu",
    target: "x86_64-unknown-linux-gnu",
    artifactNames: ["consultant-native-x86_64-unknown-linux-gnu"],
  },
  {
    key: "linux-arm64-gnu",
    target: "aarch64-unknown-linux-gnu",
    artifactNames: ["consultant-native-aarch64-unknown-linux-gnu"],
  },
] as const;

const artifactsRoot = process.env.CONSULTANT_ARTIFACTS_DIR
  ? path.resolve(process.env.CONSULTANT_ARTIFACTS_DIR)
  : path.join(repoRoot, "artifacts");

const partial = process.env.CONSULTANT_MULTIARCH_PARTIAL === "1";
const binaryName = "consultant-mcp-server";
...
if (missing.length > 0) {
  ...
  if (!partial) {
    console.error(
      "[assemble-multiarch] Fail-closed: all platforms required. Set CONSULTANT_MULTIARCH_PARTIAL=1 only for local debug."
    );
    process.exit(1);
  }
}
