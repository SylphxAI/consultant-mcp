 ...
 *
 * - Skip if main package version not on registry yet.
 * - Fail-closed if platform packages missing after main is published.
...
const platformNames = Object.keys(optional).filter((n) =>
  n.startsWith("@sylphx/consultant-mcp-")
);
...
if (!packageExists("@sylphx/consultant-mcp", version)) {
  console.log(
    `[verify-multiarch-readback] SKIP: @sylphx/consultant-mcp@${version} not on registry yet`
  );
  process.exit(0);
}
