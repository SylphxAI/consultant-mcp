...
HTTP_TRANSPORT="${ROOT}/crates/consultant-mcp-server/src/http_transport.rs"
...
if [[ ! -f "${HTTP_TRANSPORT}" ]]; then
	report_violation "missing crates/consultant-mcp-server/src/http_transport.rs"
fi
...
if (entry.state !== "authority_rust") {
  console.error(
    `[check-no-ts-http-backend] transport/web-mcp-http is ${entry.state}; expected authority_rust`
  );
  process.exit(1);
}
NODE
...
if [[ -f "${BIN}" ]]; then
	...
	if ! grep -q 'MCP_TRANSPORT=http' "${BIN}"; then
		report_violation "bin/sylphx-consultant-mcp must route MCP_TRANSPORT=http to Rust"
	fi

	if grep -qE 'http.*node|exec node|use_ts_transport|CONSULTANT_MCP_TRANSPORT:-}" == "ts"' "${BIN}"; then
...
if [[ "${violations}" -gt 0 ]]; then
	...
	echo "Authority: crates/consultant-mcp-server/src/http_transport.rs via bin/sylphx-consultant-mcp."
	exit 1
fi
