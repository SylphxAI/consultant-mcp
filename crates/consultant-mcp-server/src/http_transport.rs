//! Streamable HTTP Web MCP transport for consultant-mcp (rmcp).
//!
...
impl HttpConfig {
    pub fn from_env() -> Self {
        let host = std::env::var("MCP_HTTP_HOST")
            .or_else(|_| std::env::var("CONSULTANT_MCP_HTTP_HOST"))
            .unwrap_or_else(|_| DEFAULT_HOST.to_string());

        let port = std::env::var("MCP_HTTP_PORT")
            .or_else(|_| std::env::var("CONSULTANT_MCP_HTTP_PORT"))
            .ok()
            .and_then(|value| value.parse().ok())
            .unwrap_or(DEFAULT_PORT);

        let api_key = std::env::var("MCP_API_KEY")
            .or_else(|_| std::env::var("CONSULTANT_MCP_API_KEY"))
            .ok()
            .filter(|value| !value.is_empty());

        let cors_origin = std::env::var("MCP_CORS_ORIGIN")
            .or_else(|_| std::env::var("CONSULTANT_MCP_CORS_ORIGIN"))
            .ok()
            .filter(|value| !value.is_empty());
...
pub fn transport_from_env() -> Option<&'static str> {
    let transport = std::env::var("MCP_TRANSPORT")
        .or_else(|_| std::env::var("CONSULTANT_MCP_TRANSPORT"))
        .ok()?;
    if transport == "http" {
        Some("http")
    } else {
...
pub async fn serve_http(config: HttpConfig) -> anyhow::Result<()> {
    ...
    if let Some(api_key) = shared_config.api_key.as_deref() {
        let _ = api_key;
        eprintln!("[consultant-mcp] API key authentication enabled (X-API-Key header)");
    } else if !shared_config.is_loopback_host() {
        eprintln!(
            "[consultant-mcp] WARNING: bound to non-loopback host {} with no API key. \
             Set MCP_API_KEY or bind MCP_HTTP_HOST=127.0.0.1.",
            shared_config.host
        );
    }
    if let Some(origin) = shared_config.cors_origin.as_deref() {
        eprintln!("[consultant-mcp] CORS allowed origin: {origin}");
    }
